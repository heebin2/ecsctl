...
// clusterFlag/profileFlag/regionFlag는 여러 명령에서 공통으로 쓰는 --cluster/--profile/--region 값이다.
var (
	clusterFlag string
	profileFlag string
	regionFlag  string
...
var rootCmd = &cobra.Command{
	...
	Long: "ecs는 AWS ECS 서비스 상태 조회, 실시간 로그 추적, CodePipeline 배포 상태\n" +
		"확인을 터미널에서 빠르게 처리하는 CLI다.\n" +
		"리전은 --region 플래그, AWS_REGION 환경변수, 프로필의 region 설정 순으로 해석한다.",
	Example: "  ecs list\n" +
	...
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		...
		c, err := awsclient.New(cmd.Context(), profile, region)
		if err != nil {
			return err
		}
		clients = c
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&regionFlag, "region", "r", "", "AWS 리전 (미지정 시 AWS_REGION 환경변수 또는 프로필 설정 사용)")
}
