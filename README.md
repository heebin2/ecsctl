...
- **리전**: `--region` 플래그 > `AWS_REGION`/`AWS_DEFAULT_REGION` 환경변수 > 프로필의 `region` 설정 순으로 해석 (한 번 `--region`으로 지정하면 저장되어 다음부터 자동 사용)
...
| `-r, --region` | AWS 리전 (미지정 시 `AWS_REGION` 환경변수 또는 프로필 설정 사용) |
...
# ~/.aws/ecs-tools.yml
profile: default
region: ap-northeast-2   # --region 으로 지정한 경우에만 저장됨 (선택)
cluster: my-cluster
```
